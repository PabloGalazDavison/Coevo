...
# ROSETTA
my $FRAGMENT_PICKER = "/home/mora/Documents/programs/ro/source/bin/fragment_picker.static.linuxgccrelease";
my $FRAGMENT_PICKER_NUM_CPUS = 1;    # number of processors to use
my $ROSETTA_DATABASE = "$Bin/../../database"; # rosetta database
my $VALL = "$Bin/vall.jul19.2011"; # template database

# BLAST path (Requires non-blast+ NCBI version)
my $BLAST_DIR = "$Bin/blast";
my $BLAST_NUM_CPUS = 1;    # number of processors to use (blastpgp -a option)
...
my $SLAVE_LAUNCHER = "";
my $SLAVE_LAUNCHER_MAX_JOBS = 5;    # depends on your available machines/cpus
...
my $INSTALL_DEPENDENCIES_DATABASE = "uniref50"; # "uniref50" # "uniref90"
...
my $PSIBLAST = "$BLAST_DIR/bin/blastpgp";
...
if (!-s $FRAGMENT_PICKER) {
  ...
  $FRAGMENT_PICKER =~ s/fragment_picker[^\/]*(\.[^\.]+)$/fragment_picker$1/;
	if (!-s $FRAGMENT_PICKER) {
		$FRAGMENT_PICKER =~ s/fragment_picker(\.[^\.]+)$/fragment_picker.mpi$1/;
	}
...
# Function to trim the phipsi file based on the length of the sequence
sub trim_phipsi_file {
    my ($input_file, $output_file, $sequence) = @_;

    # Calculate the number of lines to keep (length of the sequence)
    my $num_lines_to_keep = length($sequence);
    ...
    for my $i (0 .. $num_lines_to_keep - 1) {
        print $out_fh $lines[$i];
    }
    close $out_fh;
}
...
if ($SPARKS) {
    unless ( &nonempty_file_exists( $options{fastafile} . ".phipsi" ) ) {
        ...
        # Trim the phipsi file after sparks execution
        my $input_phipsi_file = $options{fastafile} . ".phipsi";
        my $output_phipsi_file = $options{fastafile} . ".phipsi";
        #trim_phipsi_file($input_phipsi_file, $output_phipsi_file, $sequence);

        # Update the script to use the new trimmed phipsi file
        #$options{fastafile} = $output_phipsi_file;
    }
}
use Data::Dumper;
...
print "Length of the sequence: ", length($sequence), "\n";

# Debugging: Print the contents of the input sequence file
print "Contents of the input sequence file ($options{fastafile}):\n";
print $sequence, "\n";
...
unless ( &nonempty_file_exists("$options{runid}.checkpoint") ) {
    ...
    # Debugging: Print the contents and dimensions of @checkpoint_matrix after parsing
    print "After parsing, contents of \@checkpoint_matrix:\n";
    #print Dumper(\@checkpoint_matrix);
    print "Number of rows: ", scalar(@checkpoint_matrix), "\n";
    print "Number of columns: ", scalar(@{$checkpoint_matrix[0]}), "\n" if @checkpoint_matrix;

    # Finish the checkpoint matrix
    @checkpoint_matrix = &finish_checkpoint_matrix( $sequence, @checkpoint_matrix );

    # Debugging: Print the contents and dimensions of @checkpoint_matrix after finishing
    print "After finishing, contents of \@checkpoint_matrix:\n";
    #print Dumper(\@checkpoint_matrix);
    print "Number of rows: ", scalar(@checkpoint_matrix), "\n";
    print "Number of columns: ", scalar(@{$checkpoint_matrix[0]}), "\n" if @checkpoint_matrix;

    # Write the checkpoint matrix to a file
    &write_checkpoint_file( "$options{runid}.checkpoint", $sequence, @checkpoint_matrix );
}
...
sub trim_phipsi_file {
    ...
    # Open the input file for writing (overwrite mode)
    open my $out_fh, '>', $input_file or die "Cannot open $input_file: $!";

    # Write the first line (comment) to the output file
    print $out_fh $lines[0];

    # Write the next N lines to the output file
    for my $i (1 .. $num_lines_to_keep) {
        print $out_fh $lines[$i];
    }
    close $out_fh;
}
...
# Trim the phipsi file to match the sequence length and overwrite the original file
my $input_phipsi_file = "$options{fastafile}.phipsi";
trim_phipsi_file($input_phipsi_file, $sequence);
...
sub finish_checkpoint_matrix {
    ...
    # Print a stack trace to identify the caller
    Carp::cluck("finish_checkpoint_matrix called");
    my @sequence = split( //, $s );
    my $i;
    my $j;
    my $sum;
